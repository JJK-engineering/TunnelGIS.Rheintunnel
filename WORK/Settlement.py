...
for i in range(len_df): #start loop
    a=array([tv_df['Easting'][i], tv_df['Northing'][i], tv_df['Elevation'][i], tv_df['DTM'][i]]) # read in the values which are needed for calculation of point i
    b=array([tv_df['Easting'][i+1], tv_df['Northing'][i+1], tv_df['Elevation'][i+1], tv_df['DTM'][i+1]]) # read in the values which are needed for calculation of point i+1
    ...
    ab=array([ a[1]-a[0],b[1]-b[1] ])   #vector between point i and i+1
    n1=1.0  
    n2=(ab[0]*n1) / (ab[1])
    ...
    V_l=0.005  #assumed volume loss: 0.5 %
